...
def cbst(root, min_, max_):
    if root is None:
        return True

    if root.data > max_ or root.data < min_:
        return False

    return cbst(root.left, min_, root.data-1) and cbst(root.right, root.data+1, max_)
